        ...
        <div class="edd-reviews-form" id="edd-reviews-respond">
			<?php if ( ! edd_reviews()->maybe_restrict_form() ) { ?>
				<?php if ( is_user_logged_in() || ( ! is_user_logged_in() && edd_reviews()->is_guest_reviews_enabled() ) ) { ?>
                    ...
                    <form method="post" name="<?php echo edd_reviews()->review_form_args( 'name_form' ); ?>" id="<?php echo edd_reviews()->review_form_args( 'id_form' ); ?>" class="<?php echo edd_reviews(
					)->review_form_args( 'class_form' ); ?>">
                        <fieldset>
                            <div class="edd-reviews-form-inner">
                                ...
                                <p class="edd-reviews-review-form-submit">
                                    <input type="submit" class="edd-reviews-review-form-submit" id="edd-reviews-review-form-submit" name="edd-reviews-review-form-submit" value="<?php _e(
										'Post Review',
										'edd-reviews'
									); ?>"/>
			...
			<?php } else { ?>
				<?php if ( ! is_user_logged_in() ) { ?>
                    <p class="edd-reviews-not-allowed"><?php echo apply_filters(
							'edd_reviews_user_logged_out_message',
							sprintf( __( 'You must log in and be a buyer of this %s to submit a review.', 'edd-reviews' ), strtolower( edd_get_label_singular() ) )
						); ?></p><!-- /.edd-reviews-not-allowed -->
				<?php } elseif ( ! edd_has_user_purchased( $user_id, $post->ID ) ) { ?>
                    <p class="edd-reviews-not-allowed"><?php echo apply_filters(
							'edd_reviews_user_non_buyer_message',
							sprintf( __( 'You must be a buyer of this %s to submit a review.', 'edd-reviews' ), strtolower( edd_get_label_singular() ) )
						); ?></p><!-- /.edd-reviews-not-allowed -->
				<?php } // end if ?>

				<?php
				if ( ! is_user_logged_in() ) {
					$output = wp_login_form( [ 'echo' => true ] );
